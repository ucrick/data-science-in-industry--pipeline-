shinyServer(function(input, output, session) {
  ...
  getSplit <- reactive({
    ...
    #createDataPartition(y = getData()$Response, p = input$Split, list = FALSE, times = 25)
  })
  ...
  # METHOD * bam ---------------------------------------------------------------------------------------------------------------------------
  library(mgcv)  #  <------ Declare any modelling packages that are needed (see Method List tab)
  ...
  observeEvent(
    input$bam_Go,
    {
      ...
      clus <- startMode(input$Parallel)
      tryCatch({
        model <- caret::train(getBamRecipe(), data = getTrainData(), method = method, metric = "RMSE", trControl = getTrControl(),
